                         ...
                         float c[kI][kJ])
{
  ...
  int start = rank * kI / num_processes;
  int end = (rank + 1) * kI / num_processes;
  ...
  if (rank == 0)
  {
    ...
    {
      for (int j = 0; j < kK; ++j)
        a_contiguous[i * kK + j] = a[i][j];
    }
    for (int i = 0; i < kK; ++i)
    {
      for (int j = 0; j < kJ; ++j)
        b_contiguous[i * kJ + j] = b[i][j];
    }
  }
  ...
  printf("computing slice %d (starting at %d to %d)\n", rank, start, end - 1);
